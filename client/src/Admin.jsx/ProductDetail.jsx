...
const ProductDetail = () => {
  ...
  const fetchProduct = async () => {
    try {
      await axios.get(BASE_API + `/products/${id}`).then((res) => {
        ...
        setProduct({ ...car });
        setProductId(car.product_id);
        setName(car.product_name);
        setPrice(car.price);
        setBatteryCapacity(car.battery_capacity);
        setConsumption(car.consumption);
        setWattage(car.wattage);
        setAirBag(car.air_bag);
        setSeat(car.seat);
        setGuarantee(car.guarantee);
        setRoofColors(
          res.data.car.roof_colors.reduce((pre, cur) => {
            if (pre.some((e) => e == cur.roof_name)) {
              return pre;
            }
            return [...pre, cur.roof_name];
          }, [])
        );
      });
    } catch (error) {
  ...
  const fetchOneCar = async () => {
    await axios
      ...
      .then((res) => {
        ...
        setPrice(res.data.car[0].price);
        setCarSource(res.data.car[0].source);
        setBatteryPrice(res.data.car[0].battery_price);
        setDepositPrice(res.data.car[0].deposit_price);
      })
  ...
  useEffect(() => {
    fetchOneCar();
  }, [version, exteriorColor, roofColor]);
  const handleBack = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    axios
      .patch("http://localhost:3636/api/v1/productdetail/edit", {
        id,
        price,
        version,
        exteriorColor,
        ...
        <form className="container" onSubmit={handleSubmit}>
          ...
          <div className="row">
            <div className="col-4">
              ...
              <div className="row">{/* <FileUpload /> */}</div>
            </div>
            <div className="col-8">
